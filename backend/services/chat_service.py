...
Handles prompt construction and Groq API orchestration.
"""
import json
import logging
...
def generate_chat_response(payload: ChatMessageRequest) -> ChatMessageResponse:
    """
    Generate an AI response using Groq, enriched with user context.
    """
    ...
    # 1. Retrieve last 3 messages for history (simplified for demo)
    history_context = ""
    if user_id:
        try:
    ...
    system_prompt = (
        ...
        "4. FOLLOW-UP: Always end with exactly 3 numbered follow-up questions relevant to the current topic.\n"
        "5. SUMMARY: Start your response with a concise 1-sentence 'Quick Take' before the detailed explanation.\n"
        "6. SAFETY: Refuse any harmful, offensive, or non-educational content politely.\n"
    )
